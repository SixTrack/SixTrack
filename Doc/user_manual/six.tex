...
During tracking, when one of the PIPE FUNs are called SixTrack writes a line similar to
\texttt{GET ID=myID1 TURN=~1} to the output file.
It then waits for a single floating point number to be written (in ascii) to the input file, which is then read and returned from the FUN.
