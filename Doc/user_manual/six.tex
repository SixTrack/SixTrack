...
\begin{enumerate}
	...
	Header line 1 (alt.\ 1): \texttt{\# DUMP format \#2, bez=\textit{bez(i)}, number of particles=\textit{napx}}\\
        Here \textit{bez} is the name of the SINGLE ELEMENT.\\
	...
	If there are multiple single elements attached to the file, header line 2 is follwed by a number of lines similar to line 1, describing which elements are used and the current dump period.\\
	Data lines: \texttt{particleID turn s[m] x[mm] xp[mrad] y[mm] yp[mrad] z[mm] dE/E ktrack}
	\item[4] -- Beam mean and sigma:\\
	...
	Data lines: As described in the header; one per turn (and for collimation, one per pack of particles). The for the ``product'' quantities, the units are the product of the units of the ``normal'' ones.
\end{enumerate}
