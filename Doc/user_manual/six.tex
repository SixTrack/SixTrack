...
\begin{itemize}
  \item 2\textsuperscript{nd} order multipole (quadrupole):\\
    ...
    where \texttt{-0.06*1e-3} is the b2 value in units of $\mathrm{Tm/m^{n-1}}$.\\
  ...
  \item 3\textsuperscript{rd} order multipole (sextupole):\\
    ...
    where \texttt{1159.*1e-3} is the b3 value in units of $\mathrm{Tm/m^{n-1}}$.\\
  ...
  \item 4\textsuperscript{th} order multipole (octupole):\\
    ...
    where \texttt{-4.*1e-3} is the b4 value in units of $\mathrm{Tm/m^{n-1}}$.\\
    This gives the following single element in fort.2:\\
...
The effect on yp was linear, quadratic and cubic with y when using $b_2$, $b_3$, or $b_4$, respectively, as expected.
...
So in order for this paper's~\cite{RFmultsPaper} analytical equations to be compatible with Mad-X, the equations for normal quadrupole should read: $\Delta x'=-\frac{b_2}{(2-1)! ~ B\rho} \ldots$
