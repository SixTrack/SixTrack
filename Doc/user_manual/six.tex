...
\paragraph{Description} The wire module serves for reading in the input parameters for the wire. Each wire also needs to be added as single element in the list of single elements.
...
\begin{center}
	\begin{longtable}{|p{2.0cm} p{1.0cm} p{9.2cm}|}
		...
		\emph{flag\_co} & - &
		flag to define the displacement of the wire in respect to the closed orbit or x=y=0. \\*
		\emph{current} & A &
		wire current \\*
		\emph{int\_length} & m &
		...
		hor. tilt of the wire -90 < \emph{tilt\_x} < 90 (uses same defintion as DISP block) \\*
		\emph{tilt\_y} & degrees &
		vert. tilt of the wire -90 < \emph{tilt\_y} < 90 (uses same defintion as DISP block) \\*
		\hline
