...
deflection angle.
 ...
 \item[ed] $r_{21}$, horizontal edge focusing.
 \item[ek] $r_{43}$, vertical edge focusing. 

\paragraph{Remark}
MAD-X is outputting the correct format when using the dipedge element. An example of  the hard edge model is described in the physics guide, which gives $r_{21} = -r_{43}$. Note that the values of the vertical edge focusing is dependent on the modeling of the fringe fields \cite{dipedge}.  
