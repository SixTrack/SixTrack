...
\paragraph{Description} The electron lens module serves for reading in the input parameters for different types of electron lenses. Each e-lens also needs to be added as single element in the list of single elements. Currently the ideal electron lens is implemented, i.e.~with no errors in the e-beam distribution.

\paragraph{Keyword} ELEN

\paragraph{Number of data lines} variable

\paragraph{Format} \emph{name type theta\_r2 r2 r1 offset\_x offset\_y flag\_entrance flag\_exit $\sigma$}
...
\begin{center}
	\begin{longtable}{|p{2.25cm} | p{2.0cm} p{1.0cm} p{9.0cm}|}
		...
		& \emph{type} & - &
		type of electron lens. Available types are UNIFORM and GAUSSIAN. \\*
		%\hline

		& \emph{theta\_r2} & mrad &
		Kick received at $r=r_2$ where $r_2$ is the outer radius of the electron lens.\\*
		& \emph{r2} & mm &
		Outer radius of e-lens.\\*
		& \emph{r1} & mm &
		Inner radius of the e-lens. Can be 0 but not negative.\\*
		& \emph{offset\_x} & mm &
		horizontal offset of e-lens.\\*
		& \emph{offset\_y} & mm &
		vertical offset of e-lens.\\*
		& \emph{flag\_entrance} & - &
		enable bends at entrance of e-lens (not yet implemented).\\*
		& \emph{flag\_exit} & - &
		enable bends at exit of e-lens (not yet implemented).\\*
		\hline
		\rowcolor{blue!15}
		\multicolumn{4}{|l|}{type specific parameters} \\*
                GAUSSIAN & \emph{$\sigma$} & mm &
		Sigma of the e-beam.\\*
	\end{longtable}
\end{center}

Currently, two types of electron beam profiles are supported:
\begin{description}
\item[UNIFORM] e-beam with constant density of electrons;
\item[GAUSSIAN] e-beam with a radial Gaussian profile.
\end{description}
...
The implementation of the UNIFORM and GAUSSIAN types (ideal e-lenses) has no explicit energy-dependency, except for the user defined parameter \emph{theta\_r2} (see \cite{sixphys}).

\paragraph{Example} In the following we give some examples for e-lens definitions.
The example defines two electron lenses hel1 and hel2. The former is a hollow e-lens, with a uniform electron density for cleaning purposes; the latter has a Gaussian electron beam profile. The input block in \verb|fort.3| is then given by:
\begin{verbatim}
ELEN
...
Currently only the ideal hollow electron lens is implemented.

\bigskip
\begin{tabular}{@{}lp{0.8\linewidth}}
    \textbf{Keyword}    & \texttt{ELEN} \\
    \textbf{Data lines} & Variable \\
    \textbf{Format}     & \texttt{name type thetamax r2 r2ovr1 offset\_x offset\_y flag\_entrance flag\_exit} \\
...
\begin{center}
\begin{longtable}{|p{1.8cm} | p{2.5cm} | p{1.0cm} | p{9.0cm}|}
    %Note: \\* (with the star) used to inhibit page breaks in the middle of groups of the same type
    \caption{Input parameters for ELEN block.}
    \label{tab:elen} \\*
    \hline
    \rowcolor{blue!30}
    Type & Arguments & Unit & Description \\*
    \hline
    \endfirsthead
    ...
    & \texttt{type} & -- & Type of electron lens. Available types are ANNULAR. \\*
    \hline

    \rowcolor{blue!15}
    \multicolumn{4}{|l|}{type specific parameters} \\*

    \texttt{ANNULAR} & \texttt{thetamax} & mrad & Maximum kick. This equals the kick received at $r=r_2$ where $r_2$ is the outer radius of the electron lens.\\*
    \hline

    & \texttt{r2} & mm & Outer radius of e-lens.\\*
    \hline

    & \texttt{r2ovr1} & -- & Outer radius/inner radius.\\*
    \hline
    ...
    & \texttt{flag\_entrance} & -- & Enables bends at entrance of e-lens.\\*
    \hline

    & \texttt{flag\_exit} & -- & Enables bends at exit of e-lens (not yet implemented).\\*
\end{longtable}
\end{center}
...
The implementation of the \texttt{ANNULAR} type (ideal hollow e-lens) has no explicit energy-dependency, except for the user defined parameter \texttt{thetamax} (see \cite{sixphys}).
...
This example defines two electron lenses hel1 and hel2. The input block in \verb|fort.3| is then given by:
\begin{cverbatim}
ELEN
