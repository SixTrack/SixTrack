...
\subsection{Wire}
\label{sec:wire}
The generic formula for a vector potential of a straight current bearing wire with length $L$ and current $I$ centered at the origin is give in Cartesian coordinates by the following expression\footnote{Note that Curvilinear and Cartesian coordinate system are equivalent as the curvature is zero at the location of the wire element.}: 
...
b&=x^2+y^2+z^2,
\end{align}
and $\cos(c_i)$ are the direction cosines. The direction cosines can in general be expressed through two tilt angles $\phi$ and $\theta$ (see Fig.~\ref{fig:wire}) with:
...
\begin{figure}[t]
	...
	\draw [thick, ->] (0,-7) -- (0,7);
	\draw [thick, ->] (-7,-7) -- (7,7);
	\draw [blue, ultra thick] (-7,-3.5) -- (7,3.5);
	...
	\draw [dashed] (5.5, 2.0) -- (5.5, 0.0);
	\draw [dashed] (5.5, 0.0) -- (7.0, 1.5);
	...
	\draw [red, ultra thick, dashed] (7.0, 1.5) -- (0.0, 0.0);
	\draw [red, ultra thick, dashed] (0.0, 0.0) -- (5.5,2.0);
	%
	\draw[gray, very thick] (2.0,0.4) to [out=90,in=95] (2.1,0.0);
	\draw[gray, very thick] (1.9,0.4) to [out=90,in=95] (2.0,0.0);
	\draw[gray, very thick] (3.0,1.0) to [out=90,in=70] (3.0,0.0);
	%
	\node [below] at (2.0,0.0) {${\theta}$};
	\node [below] at (3.0,0.0) {${\phi}$};
	\end{tikzpicture}
	\caption{Wire centered at the origin including the angles $\phi$ and $\theta$ as defined for the directional cosines in Eqn.~\ref{wire:eqn:3}.\label{fig:wire}}
\end{figure}
...
\end{align}
where $L_{\mathrm{emb}}$ is the embedding drift or integration length, which takes into account the fringe filed of the wire, while the parameter $L$ in Eqn.~\ref{wire:eqn:4} denotes the physical length of the wire. A symplectic rotation of the coordinate system as described in \cite{forest99} is included in order to compute the transport map for arbitrary orientation, explicitly arbitrary angles $\theta$ and $\phi$ in Fig.~\ref{fig:wire}.  

The wire element is described by 7 parameters: 
tilt in respect of OY - $\phi$; tilt in respect of OX - $\theta$; wire current - I; wire length - L; embedding drift - Lemb; dx, dy - horizontal and vertical distances between wire midpoint and closed orbit (or origin of coordinate system).    
...
\item[Step 2] Symplectic rotation in $(x,z)$ by $\theta$ as described in \cite{forest99}: In a drift space the normalized momenta $p_i$ are related by $p_x^2+p_y^2 +p_z^2=1$  and we can thus write $p_z$ as
\begin{align}
...
$p_z = \sqrt{1-p_x^2-p_y^2}$ using the updated values of $p_x,p_y$ from Step 2. Analogical to the symplectic rotation in $(x,z)$, the rotation by $\phi$ in $(y,z)$ can be expressed as a rotation first in $(p_y,p_z)$:
\begin{align}
...
$p_z = \sqrt{1-p_x^2-p_y^2}$ using the updated values of $p_x,p_y$ from Step 4. The rotation is then given by:
\begin{align}
p_y & \to   p_y \cdot \cos \phi - p_z \cdot \sin\phi \nonumber\\
p_z & \to   p_y \cdot \sin \phi + p_z \cdot \cos\phi 
\end{align}
\item[Step 6] Backward rotation by $-\theta$ in $(p_x,p_z)$: The new $p_z$ is again defined as
$p_z = \sqrt{1-p_x^2-p_y^2}$ using the updated values of $p_x,p_y$ from Step 5. The rotation is then given by:
\begin{align}
 p_x & \to   p_x \cdot \cos \theta - p_z \cdot \sin\theta \nonumber\\
 p_z & \to   p_x \cdot \sin \theta + p_z \cdot \cos\theta 
\end{align}
