...
\textcolor{notered}{\textbf{Note:} This module is experimental! Use at your own risk.
Both the input format and physics implementation may change.}\index{scatter}\index{elastic scattering}\index{SCAT}
...
From SixTrack version 5.2.3 and on, the format of the \texttt(COLL) block is a set of keyword/value lines.
This format is described in Section~\ref(sec:coll:newfmt).

For backwards compatibility, the old format is still supported, and it is described in Section~\ref{sec:coll:oldfmt}..
When encountering the \texttt{COLL} block, the parser will check if the first item of the line is a valid keywords, and if it is not, it will fall back to parsing the block in the old format.
...
Note that the old and new format cannot be inter-mixed, as the old format is parsed based on the line number.
The 17 lines must follow exactly the format described in Table~\ref{tab:COLL_INP}.

\subsubsection{New Format}\label{sec:coll:newfmt}
...
\paragraph{Do Collimation} \texttt{DO\_COLL true|false}\\

The collimation modul can be switched on and off in its entirety with this flag without having to comment in and out the entire block.
When the flag is off, the collimation routines are skipped in tracking, and the memory needed for collimation is not allocated.

\paragraph{Collimator Database} \texttt{COLLDB filename}\\

The filename of the database of collimators.

\paragraph{Beam Energy} \texttt{ENERGY energy[MeV]}\\

The energy of the beam to be tracked.
This is independent of the energy specified in other blocks.

\paragraph{Beam Number} \texttt{BEAM\_NUM 1|2}\\

Number of the beam tracked.

\paragraph{Emittance} \texttt{EMIT eX\_d eY\_d eX\_g eY\_g}\\
...
Set the seed for the random number generator. If set to 0 (default) a seed will be selected for each run.

\paragraph{Distribution Generator Type} \texttt{DIST\_TYPE 0-6}\\

The type of the distribution generator to use.
Setting this larger than 0, will overwrite the distribution generated by the initialisation routines in SixTrack, including the distribution read from the \texttt{DIST} block (see Section~\ref{distBlock}).
...
Nominal values are $3.06 \times 10^{-4}$ at 450 GeV and $1.129 \times 10^{-4}$ at 7 TeV for \texttt{enerror}, and 11.24 cm at 450 GeV and 7.55 cm at 7TeV for \texttt{bunchlen}.

\paragraph{Distribution Generator Input File} \texttt{DIST\_FILE filename}\\

Name of the distribution file to be read if for distribution type 4 or 6. 

\paragraph{Special Radial Distribution} \texttt{DO\_RADIAL true|false size smear}\\

Alternative radial beam distribution where \texttt{size} is the size of the beam in numbers of radial sigmas, and \texttt{smear} is the smear of the beam in radial sigma.
Setting this to true will oveeride \texttt{DIST\_TYPE}.

\paragraph{Do Change Collimator Gaps} \texttt{DO\_NSIG true|false}\\

If true, the collimator gaps specified in the imput block by keyword \texttt{NSIG\_FAM} are used instead of those in the collimator database file.
...
\paragraph{Set Jaw Slicing} \texttt{JAW\_SLICE nslice smin smax recenter1 recenter2}\\

The surface model of the jaw. Specify the slicing of the jaw.

\bigskip
\begin{tabular}{@{}llp{0.7\linewidth}}
    \texttt{nslice}    & integer & Number of slices in which each jaw should be cut. \\
    \texttt{smin}      & float   & S-position for the start of the slicing. \\
    \texttt{smax}      & float   & S-position for the end of the slicing. \\
    \texttt{recenter1} & float   & Moving the 1st jaw to the new smallest opening. \\
    \texttt{recenter2} & float   & Moving the 2nd jaw to the new smallest opening.
\end{tabular}

\paragraph{Set Jaw Fits} \texttt{JAW\_FIT[1|2] 0th 1st 2nd 3rd 4th 5th scale}\\

Specifies the polynomial fit of the first or second jaw for the 0th to the 5th order, as well as the scaling of the polynomial.

\paragraph{One-Sided Collimators} \texttt{DO\_ONESIDE true|false}\\

Switches on or off the collimator being one-sided.
That is, only positive jaw.
If the negative jaw is to be used, it is necessary to turn collimator by 180 degrees in the collimator database file.
...
Does a dedicated study of selected collimator, can be switched on an off.
...
Apply erros to primary or secondary collimator tilt and offset.
...
\paragraph{Cut Halo File (CERN Flag)} \texttt{CERN true|false}\\

Switches on or off whether to cut halo files in separate pieces, one per 64 particles.
...
\subsubsection{Old Format}\label{sec:coll:oldfmt}
