...
The user can modify the default behaviour of SixTrack in considering the kick from electron lenses in the FOX calculation.
...
The user can choose to interpolate the radial profile from a file (see Sec.~\ref{sec:elen_rad_prof}) not linearly but with a polynomial of higher order, up to 19, chosing the number of points to be used for the interpolation. The default is linear interpolation, i.e.~using 2 points.
...
\begin{longtable}{| p{2.4cm} | p{1.0cm} | p{12.0cm}|}
    ...
    \end{tabular}
    \\*
    & & The default behaviour is that the current \texttt{SIGDPP} line applies only to the e-lens being declared, without touching the others. \\*
    \hline
