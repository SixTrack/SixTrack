...
From SixTrack version 5.2.5 and on, the format of the \texttt(COLL) block is a set of keyword/value lines.
...
When encountering the \texttt{COLL} block, the parser will check if the first item of the block is a single value, and if it is, it will fall back to parsing the block in the old format.
...
Alternative radial beam distribution where \texttt{size} is the amplitude of the beam in numbers of radial sigmas, and \texttt{smear} is the smear of the beam in radial sigma.
Setting this to true will override \texttt{DIST\_TYPE}.

\paragraph{Special Radial Distribution} \texttt{DO\_MINGAP true|false}\\

If true. the particle distribution is generated at the collimator with the smallest gap (to be used with sheet/pencil beam).
