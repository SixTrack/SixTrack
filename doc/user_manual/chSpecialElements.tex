...
\begin{longtable}{|p{2.0cm} | p{2.4cm} | p{1.0cm} | p{9.0cm}|}
    ...
    & \texttt{filename} & & filename of the radial profile. Please see Sec.~\ref{sec:elen_rad_prof} for proper formatting of the file. The profile is read from the file, radially integrated and normalised to the total current. Hence, the kick at \texttt{r2} is given by the value of \texttt{theta\_r2} (or by the optional parameters) and not by integration of the radial profile. The kick is obtained linearly interpolating the normalised profile. The domain of the profile must be larger than the domain identified by $R_1$ and $R_2$. \\*
    \hline
...
\begin{tabular}{@{}lp{0.8\linewidth}}
    \textbf{Format}     & \texttt{FOX true|false or on|off} \\
\end{tabular}

\bigskip
\noindent The user can modify the default behaviour of SixTrack in considering the kick from electron lenses in the FOX calculation.

\subsection{INTER statement}\label{sec:elen:INTER}
\begin{tabular}{@{}lp{0.8\linewidth}}
    \textbf{Format}     & \texttt{INTER elens\_radial\_mpoints} \\
\end{tabular}

\bigskip
\noindent The user can chose to interpolate the radial profile from a file not linearly but with a polynomial of higher order, up to 19, chosing the number of points to be used for the interpolation. The default is linear interpolation, i.e.~using 2 points.

\subsection{Example} \label{sec:elen:example}

In the following we give an example of e-lens definition.
The example defines three electron lenses, i.e.~ \texttt{hel1}, \texttt{hel2} and \texttt{hel3}; the first one has a uniform electron density, whereas the second one has an electron beam with a Gaussian profile with $\sigma$=1.1547~mm and the third one makes use of an ASCII file for the radial distribution.
While the first and third e-lenses are hollow, the second one is full.
While the first and third e-lenses have an explicit definition of the kick at \texttt{r2}, the second one triggers the re-computation of the kick, given the length of the lens of 4~m, the electron current of 2~A (opposite wrt the main beam) and the electron kinetic energy of 1~keV.
...
FOX false
hel2 GAUSSIAN 4.920e-03 6.928 0.0 1.1547 2.3093 1.1547 4 -2 1
...
\noindent An example of radial profile is contained in the test suite of e-lenses.
