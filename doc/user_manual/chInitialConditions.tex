...
The \texttt{SIMU} block (Section~\ref{Input:SIMU}) can currently be used to replace the \texttt{TRAC} block (Section~\ref{TraPar}), but note that the implementation may still have bugs, and is therefore considered experimental\index{SIMU}.
...
\bigskip
The Simulation block (\texttt{SIMU}) is intended to take the main simulation parameters, but currently only replaces the \texttt{TRAC} block described in Section~\ref{TraPar}\index{SIMU}\index{Simualtion Block}.
...
\begin{tabular}{@{}llp{0.7\linewidth}}
    ...
    \textbf{Format}     & Name/value format. See Table~\ref{Table:SIMU}.
\end{tabular}
...
\begin{longtable}{@{\extracolsep{\fill}}|l|p{10cm}|l|}
    \caption{Available function types in the SIMU block.}
    \label{Table:SIMU} \\*
    \hline
    \rowcolor{blue!30}
    ...
    \hline
    \rowcolor{gray!15}
    \texttt{INIT\_CLORB} & \texttt{x xp y yp [sigma dpsv]} & \texttt{6 * 0.0} \\*
    \hline
    \multicolumn{3}{|>{\raggedright}p{\textwidth}|}{%
        Initialise closed orbit.
        This can be called either with a flag, in which case it turns on or off the reading of a closed orbit suggestion from file \texttt{fort.33}, or it can provide 4 or 6 values for the closed orbit suggestion.
    ...
    \multicolumn{3}{|>{\raggedright}p{\textwidth}|}{%
        ...
        The \texttt{rewind} flag specifies whether or not to rewind the tracking files on each write.
