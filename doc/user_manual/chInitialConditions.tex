...
In addition to \texttt{fort.13}, it is possible to read a beam distribution from a plain ASCII file via the \texttt{DIST} block.
With respect to using \texttt{fort.13}, which specifies just the six coordinates, the \texttt{DIST} block allows the user to specify further parameters of the particles, like charge or mass, necessary in case the user wants to track species different than the synchronous one.
The user has to specify the file from which the code should read the distribution.
...
Table~\ref{tab:distKeys} reports the available keywords and meaning of necessary values.
Table~\ref{tab:distReadFileColumns} shows the format of the ASCII file, along with meaning of columns and units when necessary.
