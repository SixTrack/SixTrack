...
for fnin,fnout,offx,offy,R in [(1,2,0,0,0.5),(2,3,offsetx,offsety,1),(3,4,-offsetx,0,1/3.),(4,5,0,-offsety,1/4.)]:
  ...
  if np.max(rrin-rrout)==0:
    ...
    plt.plot(rrin/sig,np.ones(len(rrin))*theta_max,'k-',label='theta_r2')
...
plt.figure('elens kick',figsize=(10,10))
for name,fnin,R in zip(names[1:],[1,2,3,4],[0.5,1,1/3.,1/4.]):
  theta_max=theta_r2*R
  plt.subplot(2,2,fnin)
