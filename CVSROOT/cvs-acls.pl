...
eval 'exec perl -S $0 ${1+"$@"}'
    if 0;
#
# $Id: cvs-acls.pl,v 1.1.1.1 2004-07-15 09:42:15 slopiens Exp $
#
...
# Also try to strip off these at the beginning of the repository name.
# Found necessary to overcome a tagging problem many people have.
# H. Meinhard   27-Jul-2001
#@myroots = ("/local/atlas/cvs", "/atlascvs");

$debug = 0;
$cvsroot = $ENV{'CVSROOT'};
$availfile = shift;
$myname = getlogin || (getpwuid($<))[0] || $ENV{"LOGNAME"} || $ENV{"USER"} || "nobody";
