  ...
  if(ncheby /= 0) then
    do j=1,nele
      if(kz(j) == cheby_kz) then
        if( icheby(j) == 0) then
          write(lerr,"(a)") "CHEBY> ERROR Chebyshev lens element '"//trim(bez(j))//"'not defined in fort.3."
          write(lerr,"(a)") "CHEBY>       You must define every Chebyshev lens in the CHEB block."
          iErr = .true.
          return
        end if
      end if
