...
c      IF (abs(xp_rel) .lt. xpcrit) THEN              ! if R' < R'c (ok CH) (1)
c        Chann  = (xpcrit**2-xp_rel**2)**0.5/xpcrit0  ! probability of CH/VC  OCCHIO DANIELE LINEA COMMENTATA PER PROVARE LA SUCCESSIVA PROB.
c        N_atom = 0.1                                ! probability of entering OCCHIO DANIELE, prob cambiata in accordo con nuova chann
c--------------DAN CHAN prob------
c         alpha = xp_rel/xpcrit
c         Chann = ((0.64-(1/ratio)*(1/ratio)*0.64)**0.5)*     !DANIELE saturation at 80%
c     &           (1-alpha*alpha)**0.5  

c         Chann = ((0.8-(1/ratio)*(1/ratio)*0.8)**0.5)*     !DANIELE saturation at 90%
c     &           (1-alpha*alpha)**0.5       

c         N_atom=0.14                                      !DANIELE for sat. at 80% 
c         N_atom=0.1                                       !DANIELE for sat. at 90%

c      IF (abs(xp_rel) .lt. xpcrit) THEN            
c         Chann = ((0.9-(1/ratio)*(1/ratio)*0.9)**0.5)*     !DANIELE saturation at 95%
c     &           (1-alpha*alpha)**0.5       
c        N_atom = 0.1
...
c      IF (abs(xp_rel) .lt. xpcrit) THEN  
c         alpha = xp_rel/xpcrit  
c         Chann = ((0.8-alpha*alpha*0.8)**0.5)*(1-(1/ratio))**0.5         !DANIELE saturation at 90%
c         N_atom = 0.1  
...
!     +          xp,yp,PC)
     ...
     +          xp,yp,PC)
          ...
          ENDIF                        
C.  case 3-3: move in amorphous substance (big input angles)---------------  MODIFIED FOR TRANSITION VRAM DANIELE
        else
           if(xp_rel .GT. L_chan/Rcurv+2.0*xpcrit .or. 
     +        xp_rel .lt. -xpcrit) then                          
             PROC='AM'
             x = x + 0.5 * s_length * xp
             y = y + 0.5 * s_length * yp
            if(ZN .gt. 0) then                        
...
c            Pvr=0.5*erf((xp_rel-(L_chan/Rcurv)-xpcrit)/
c     +          (2.0*4.0*xpcrit*xpcrit)**0.5)+0.5
c            Pvr=0.2+(0.6/(2.0*xpcrit))*(xp_rel-(L_chan/Rcurv))
            Pvr=((xp_rel-(L_chan/Rcurv))/(2.0*xpcrit))
            if(rndm4() .gt. Pvr) then
            PROC='TRVR'
            x = x + xp * Srefl
            y = y + yp * Srefl
c            Dxp=(2.0*Ang_avr-Ang_rms)/(2.0*L_chan/Rcurv+2.0*xpcrit)*
c     +         xp_rel+L_chan/Rcurv*(2.0*Ang_avr-Ang_rms)/
c     +         (2.0*L_chan/Rcurv+2.0*xpcrit)-Ang_avr
            Dxp=-3.0*Ang_rms*xp_rel/(2.0*xpcrit)+Ang_avr+
     +          (3.0*Ang_rms*(L_chan/Rcurv)/(2.0*xpcrit))
...
c            xp=xp+Dxp+Ang_rms*RAN_GAUSS(1.)
     ...
     +          DLRi(IS),xp ,yp,PC)
            ...
            x = x + xp * Srefl
            y = y + yp * Srefl
c            Dxp=-1.0*(13.6/PC)*SQRT(s_length/DLRi(IS))*xp_rel/
c     +      (2.0*xpcrit)+(13.6/PC)*SQRT(s_length/DLRi(IS))*
c     +      (1.0+(L_chan/Rcurv)/(2.0*xpcrit))
...
c            write(*,*) xp_rel, Dxp
c            xp=xp+Dxp+(13.6/PC)*SQRT(s_length/DLRi(IS))*RAN_GAUSS(1.)
            xp=xp+Dxp
            x=x+0.5*xp*(s_length-Srefl)
            y=y+0.5*yp*(s_length-Srefl)
            CALL CALC_ION_LOSS_CRY(IS,PC,s_length-Srefl,DESt)
