...
!write the aperture to root
     ...
     select case(aptype)
     case(-1) ! transition
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(1), ape(2), ape(3), &
 &                              ape(4), ape(5), ape(6), ape(7), ape(8), ape(9) )
     case(0) ! not an aperture marker
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(1), ape(2), ape(3), &
 &                              ape(4), ape(5), ape(6), ape(7), ape(8), ape(9) )
     case(1) ! Circle
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(1), zero, zero, zero,&
 &                              zero, zero, ape(7), ape(8), ape(9) )
     case(2) ! Rectangle
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(1), ape(2), zero, zero,&
 &                              zero, zero, ape(7), ape(8), ape(9) )
     case(3) ! Ellipse
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(3), ape(4), zero, zero,&
 &                              zero, zero, ape(7), ape(8), ape(9) )
     case(4) ! Rectellipse
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(1), ape(2), ape(3), &
                                ape(4), zero, zero, ape(7), ape(8), ape(9) )
     case(5) ! Octagon
        ! get angles from points passing through x1,y1 and x2,y2
        ! x1=ape(1)
        ! y1=ape(1)*tan(theta1)
        ! x2=ape(2)/tan(theta2)
        ! y2=ape(2)
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(1), ape(2), &
 &           atan2_mb(ape(1)*ape(5)+ape(6),ape(1)), atan2_mb(ape(2),(ape(2)-ape(6))/ape(5)), zero, zero, ape(7), ape(8), ape(9) )
     case(6) ! Racetrack
        call root_DumpAperture( name, len_trim(name), apeName(aptype), len_trim(apeName(aptype)), spos, ape(1), ape(2), ape(3), &
 &                              zero, zero, zero, ape(7), ape(8), ape(9) )
     end select
     end if
#endif
  end if
