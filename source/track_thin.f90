      ...
      case (1)
        ...
        if(iexact) then ! exact drift?
          do j=1,napx
            ...
            ! yv1(j)=yv1(j)*c1e3
            ! yv2(j)=yv2(j)*c1e3
            pz     = sqrt(c1e6 - (yv1(j)**2 + yv2(j)**2))*c1m3
            xv1(j) = xv1(j) + stracki*(yv1(j)/pz)
            xv2(j) = xv2(j) + stracki*(yv2(j)/pz)
          end do
        else
        ...
        if(do_coll .and. is_coll) then
          ...
          if(iexact) then
            ...
            do j=1,napx
              ...
              ! write(*,*) "OLD>",pz,rvv(j)/pz,one-(rvv(j)/pz),stracki*(one-(rvv(j)/pz))
              ! write(*,*) "OLD>",xv1(j),xv2(j),yv1(j),yv2(j),sigmv(j)
              ! stop
              ! write(*,*) "BEF>",xv1(j),xv2(j),yv1(j),yv2(j),sigmv(j)
              pz       = sqrt(c1e6 - (yv1(j)**2 + yv2(j)**2))*c1m3
              xv1(j)   = xv1(j)   + stracki*(yv1(j)/pz)
              xv2(j)   = xv2(j)   + stracki*(yv2(j)/pz)
              sigmv(j) = sigmv(j) + stracki*((one - rvv(j)/pz)*c1e3)
              ! write(*,*) "NEW>",pz,stracki*(yv1(j)/pz),stracki*(yv2(j)/pz),stracki*((one-(rvv(j)/pz))/c1e3)
              ! write(*,*) "NEW>",pz,rvv(j)/pz,c1m3-(rvv(j)/pz),stracki*(c1m3-(rvv(j)/pz))
              ! write(*,*) "NEW>",xv1(j),xv2(j),yv1(j),yv2(j),sigmv(j)
              ! stop
              ! OLD>  0.99999999998462896       -5.5445544555307704E-003   0.0000000000000000        3.8622270544019788E-003
              ! NEW>   999.99999998462897       -5.5445544555307711E-006   0.0000000000000000        9.9900386222705434E-004

              ! OLD>  0.99999999998462896       0.99613777294559802        3.8622270544019788E-003   3.8622270544019788E-003
              ! NEW>   999.99999998462897        9.9613777294559796E-004   3.8622270544020655E-006   3.8622270544020655E-006
              ! NEW>  0.99998462883980899       0.99615308495893007      -0.99515308495893007      -0.99515308495893007
              ! NEW>   999.99999998462897        9.9613777294559796E-004   3.8622270544020655E-006   3.8622270544020655E-006
              ! NEW>   999.99999998462897        9.9613777294559796E-004  0.99900386222705440       0.99900386222705440

              ! OLD>  0.39445544554446926  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8622270544019788
              ! NEW>  0.39445544554446926  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8622270544019788
              ! NEW>  0.39445544554446926  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8622270544019788
              ! NEW>  0.39445544554446926  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8622270544020654
              ! NEW>  0.39445544554446926  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8622270544020654
              ! NEW>  0.39445544554446926  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8622270544020655E-003
              ! NEW>  0.39445544554446926  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  999.00386222705436
              ! NEW>  0.39445536031700973  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8469150410699315
              ! NEW>  0.39445536031700973  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8469150410699315E-003
              ! NEW>  0.39999445544554452  0.0000000000000000  -5.5445544554455451E-003  0.0000000000000000  3.8622270544020655E-006
            end do
          else
            do j=1,napx
              xv1(j)   = xv1(j) + stracki*yv1(j)
