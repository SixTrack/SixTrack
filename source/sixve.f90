...
!  SECTOR MAGNET VERTICAL
    ...
    do 70 j=1,napx
      ...
      si=sin_mb(fok)
      co=cos_mb(fok)
      rhoc=(rho*(one-co))/dpsq(j)                           !hr06
      siq=si/dpsq(j)
      al(1,ih1,j,l)=co
      al(2,ih1,j,l)=rho*si
      al(3,ih1,j,l)=(-one*si)/rho                              !hr06
      al(4,ih1,j,l)=co
      al(5,ih1,j,l)=((-one*dpsv(j))*rhoc)*c1e3                    !hr06
      al(6,ih1,j,l)=((-one*dpsv(j))*siq)*c1e3                     !hr06
...
120     continue
    ...
    do 130 j=1,napx
      ...
      if(fok.lt.(-one*pieni)) then                                !hr06
        si=sin_mb(fi)
        co=cos_mb(fi)
        wfa(j)=((wf/afok(j))*(one-co))/dpsq(j)                 !hr06
  ...
  &sm12+ dpsv(j)*(el(l)-al(2,ih1,j,l))))/afok(j))*wf**2)*c1e3   !hr06
...
!--DEFOCUSSING
      if(fok.gt.pieni) then
        ...
        al(3,ih1,j,l)=hs*hi
        al(4,ih1,j,l)=hc
        wfa(j)=((wf/afok(j))*(one-hc))/dpsq(j)                 !hr06
  ...
  &+dpsv(j)*(el(l)-al(2,ih1,j,l))))/afok(j))*wf**2+el(l)*         &
  &(one-rvv(j)))*c1e3
      as(2,ih1,j,l)=(-one*rvv(j))*(((dpsv(j)*wf)/(two*dpsq(j)))* &!hr06
  &sm12-dpd(j)*wfhi(j))                                            !hr06
