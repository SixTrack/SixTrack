...
subroutine part_writeState(fileName, isText, withIons)
  ...
  if(isText) then
    ...
    do j=1,npart
      ...
      if(withIons) then
        ...
        write(fileUnit, "(i8,1x,i8,1x,i8,a1,i1,2(1x,l4),1x,i8,a225,3(1x,i4),1x,i11)") &
          partID(j),parentID(j),pairID(1,j),".",pairID(2,j),llostp(j),isPrim,min(numxv(j),numx+1),roundBuf(1:225),&
          naa(j),nzz(j),nqq(j),pdgid(j)
      else
        write(fileUnit, "(i8,1x,i8,1x,i8,a1,i1,2(1x,l4),1x,i8,a200)") &
          partID(j),parentID(j),pairID(1,j),".",pairID(2,j),llostp(j),isPrim,min(numxv(j),numx+1),roundBuf(1:200)
      end if
    end do
  else
    ! Format
    ...
    do j=1,npart
      ...
      write(fileUnit) iLost,iPrim,min(numxv(j),numx+1),0_int32      ! 4x32 bit
