...
subroutine abend(cstring)
  ...
  call units_openUnit(unit=10,file="fort.10",formatted=.true.,mode="r",err=fErr,status="old",recl=8195)
  if(fErr) goto 11
...
11 continue
  ...
  ! call units_openUnit(unit=10,file="fort.10",formatted=.true.,mode="w",err=fErr,status="unknown",recl=8195)
