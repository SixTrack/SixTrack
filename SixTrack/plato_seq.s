...
!C.........................................BISECTION PROCEDURE
      DO JIT=1,JITER
        ...
        DO J=1,JMAX
          ...
          DO N=1,MAX
+if crlibm
            ZC=(X(N)-(ZERO,ONE)*PX(N))
     .        *(ONE+COS_MB(STEP*(2*N-MAX1)))
            TMPR=REAL((-(ZERO,ONE)*OME)*N)
            TMPI=AIMAG((-(ZERO,ONE)*OME)*N)
            Z(N)=ZC*(EXP_MB(TMPR)*CMPLX(COS_MB(TMPI),SIN_MB(TMPI)       &
     &,fPrec)) !exp_mb is only defined for real numbers -> decompose in real and imaginary part
+ei
+if .not.crlibm
            ZC=(X(N)-(ZERO,ONE)*PX(N))
     .        *(ONE+COS(STEP*(2*N-MAX1)))
...
!C..COMPUTATION OF SCALAR PRODUCT WITH ITERATED BODE ALGORITHM
          ...
          DO I=0,MBODE
            K=4*I
            FOME=FOME+((SEVEN*(Z(1+K)+Z(5+K))+32.0_fPrec*(Z(2+K)        &
     &+Z(4+K)))+12.0_fPrec*Z(3+K))
          ENDDO
