...
!-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
!      
!     A.Mereghetti, D.Sinuela Pastor and P.Garcia Ortega, for the FLUKA *FOX
!     K. Sjobak, BE-ABP/HSS
...
!         by calculating the matrix of eigenvectors (tas)
          do j=1,ndimf
            ...
            jj(i3)=0
!    store tas matrix (normalisation of phase space) and closed orbit for FMA an
!    units dump_tas: mm,mrad,mm,mrad,mm,1.e-3
            if(fma_flag) then
              if(ic(i)-nblo.gt.0) then !check if structure element is a block
                if(ldump(ic(i)-nblo)) then !check if particles are dumped at thi
...
!    closed orbit in canonical variables x,px,y,py,sig,delta [mm,mrad,mm,mrad,mm
